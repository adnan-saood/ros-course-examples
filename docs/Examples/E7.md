...
Here's the enhanced tutorial, now with the additional steps to **extract and display specific facial features** like the eyes and mouth during real-time webcam detection using **dlib**:

---

## Tutorial: Face Detection and Feature Extraction Using dlib in Python (Real-Time Webcam with Eyes and Mouth Extraction)

### Introduction
In this tutorial, we will expand on the use of **dlib** for face detection and real-time facial feature extraction, including **eyes and mouth**. We will build on previous sections, detecting faces and landmarks from a live webcam feed, and then isolate and display specific features like the eyes and mouth.

### Prerequisites
Ensure that you have Python installed and have installed the following libraries:
- `dlib`
- `opencv-python`
- `imutils`
...
### Step 2: Detect Faces and Facial Landmarks
Download the pre-trained facial landmark predictor (`shape_predictor_68_face_landmarks.dat`) from [here](http://dlib.net/files/shape_predictor_68_face_landmarks.dat.bz2), and extract the `.dat` file into your working directory.

First, let's set up the real-time webcam detection for faces and landmarks as before.
...
### Step 3: Extracting Eyes and Mouth from Facial Landmarks
