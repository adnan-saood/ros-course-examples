...
- Attributes: `<link>`: Name of the parent link.

##### `<child>`

- Specifies the child link of the joint.
- Attributes: `<link>`: Name of the child link.
##### `<origin>`
...
###### revolute
Rotational motion with limits.
continuous
Rotational motion without limits.
###### prismatic
Translational motion with limits.
###### fixed
No motion.
###### floating
Six degrees of freedom (not commonly used in simple robots).
###### planar
Constrains motion to a plane (two translational and one rotational DOF).
